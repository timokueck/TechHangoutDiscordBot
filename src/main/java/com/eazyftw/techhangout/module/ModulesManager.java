...
import com.eazyftw.techhangout.util.ProjectUtil;
...
public class ModulesManager {
    ...
    public void load() {
        for (Class<?> each : ProjectUtil.getClasses("com.eazyftw.techhangout.module")) {
            if (CommandModule.class.isAssignableFrom(each) && !Modifier.isAbstract(each.getModifiers())) {
                try {
                    CommandModule module = (CommandModule)each.getConstructor(TechHangoutBot.class).newInstance(TechHangoutBot.getBot());
                    module.enable();
                ...
                try {
                    Module module = (Module) each.getConstructor(TechHangoutBot.class).newInstance(TechHangoutBot.getBot());
                    module.enable();
                    modules.add(module);
                } catch (InstantiationException | IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
                    e.printStackTrace();
        ...
        TechHangoutBot.getJDA().addEventListener(modules.toArray());
        TechHangoutBot.getJDA().addEventListener(cmdModules.toArray());
    ...
    public void logLoad() {
        ...
        TechHangoutBot.log("Modules:");
        TechHangoutBot.log("  » All: " + (modules.size() + cmdModules.size()));
        TechHangoutBot.log("  » Modules: " + modules.size());
        TechHangoutBot.log("  » Command: " + cmdModules.size());
        TechHangoutBot.log("  » Success: " + successfulAmount);
        TechHangoutBot.log("  » Failed: " + failedModules);
    }
